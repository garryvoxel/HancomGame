    ...
    <div class="page-wrapper page-community friends">
      ...
      <section class="main-content">
        ...
        <div class="sub-wrap p-3">
          <ul class="nav-tabs1">
              <li class="current"><router-link to="/friends/list"><div>친구목록 <span>{{friends.length}}</span></div></router-link></li>
            ...
            <ul class="friends-list">
              ...
              >
                ...
                <div class="friends-info">
                  <p class="nick">닉네임 : <span>{{friend.nickname}}</span></p>
                  <p class="lv">소속 클랜 : <span>{{friend.clan ? friend.clan : '클랜 없음'}}</span></p>
                </div>
                <div class="friends-btn">
                  <button
                    class="btn btn-secondary btn-xs"
                    @click="deleteFriend(friend.nickname)"
                  >삭제</button>
                </div>
              </li>
              <li class="friends-item friends-item-empty" v-if="friends.length <= 0">
                <div>{{alert}}</div>
  ...
  methods: {
    Log() {
      this.$axios
        .put(
          this.$Api.logs,
          { menu_type: 15, Authorization: "Bearer" + this.$root.sessionId() },
    ...
    findFriend() {
      ...
      this.$axios
        ...
        .then(response => {
          ...
          if (!response.data.code === Result.NICKNAME_IS_TOO_SHORT.code) {
