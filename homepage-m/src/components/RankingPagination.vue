<template>
    <div id="rankingpagination" class="paging">
        ...
        <span v-for="n in (endPage - startPage + 1)">
            <a v-if="parseInt(current) !== n + startPage - 1" @click.prevent="page(n + startPage - 1)">{{ n + startPage - 1 }}</a>
...
export default {
    ...
    methods: {
        ...
        page(n) {
            ...
            if(this.$route.name.match('personal')){
                ...
                // this.$store.commit('start_Rank',n-1)
                // this.$store.commit('user_Rank',(n*this.$store.state.max_Count)-this.$store.state.max_Count)
                ...
                console.log(n*this.$store.state.max_Count)
                this.$store.commit('user_Rank',(n*this.$store.state.max_Count)-this.$store.state.max_Count)
                this.$store.commit('page_direction','page');
                this.$router.push({ query: { page: n } });
            }else{
                // this.$store.commit('school_start_Rank',n-1)
                // this.$store.commit('school_user_Rank',(n*this.$store.state.school_max_Count)-this.$store.state.school_max_Count)
                // this.$store.commit('school_page_direction','page');
                // this.$router.push({ query: { page: n } });
                this.$store.commit('school_start_Rank',(n*this.$store.state.school_max_Count)-10)
                this.$store.commit('school_user_Rank',(n*this.$store.state.school_max_Count)-this.$store.state.school_max_Count)
                this.$store.commit('school_page_direction','page');
                this.$router.push({ query: { page: n } });
            }
        ...
        prev() {
            if(this.current <= 1){
                ...
                if(this.$route.name.match('personal')){
                    ...
                    this.$store.commit('start_Rank',this.$store.state.start_Rank-this.$store.state.max_Count)
                    
                    this.$store.commit('user_Rank',(this.$store.state.first_user_Rank-this.$store.state.max_Count)-1);
                    this.$router.push({ query: { page: parseInt(this.current) - 1 } });
                }else {
                    console.log('school')
                    this.$store.commit('school_page_direction','prev');
                    this.$store.commit('school_start_Rank',this.$store.state.school_start_Rank-this.$store.state.school_max_Count)
                   
                    this.$store.commit('school_user_Rank',(this.$store.state.school_first_user_Rank-this.$store.state.school_max_Count)-1);
                    this.$router.push({ query: { page: parseInt(this.current) - 1 } });
                }
            }
        ...
        next() {
            ...
            if(this.current >= this.totalPageCount){
                ...
                if(this.$route.name.match('personal')){
                    ...
                    this.$store.commit('school_page_direction','page');
                    this.$store.commit('school_start_Rank',this.$store.state.school_start_Rank+this.$store.state.school_max_Count)
                    this.$store.commit('school_user_Rank',this.$store.state.school_last_Rank)
                    this.$router.push({ query: { page: parseInt(this.current)  + 1 } });
                }
            }
        },
    ...
    computed: {
        ...
        totalPageCount() {
            ...
            return _count
            // return this.totalCount > 0 ? Math.ceil(this.redis_total_count /this.totalCount) :1;
        },
