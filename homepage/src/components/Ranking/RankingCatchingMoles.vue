<template>
  <div id="ranking-catching-moles">
    <nav v-if="$parent.isRankingOfCatchingMoles" class="tab-rounded">
      <ul>
        <li
          :class="{
            selected: $route.name === 'ranking-catching-moles-personal',
          }"
        >
          <router-link :to="{ name: 'ranking-catching-moles-personal' }" title="개인 순위">
            <span class="tab-icon icon-person"></span>
            <span>개인 순위</span>
          </router-link>
        </li>
        <li :class="{ selected: $route.name === 'ranking-catching-moles-school' }">
          <router-link :to="{ name: 'ranking-catching-moles-school' }" title="학교 순위">
    ...
    <div v-if="$route.name === 'ranking-catching-moles-personal'" class="ranking-content-wrapper">
          ...
          <thead>
            ...
            <tr v-if="isLogin" class="my-ranking">
              <td
                ...
                class="ranking"
              >{{ myRanking.rank > 0 ? myRanking.rank : '--' }}</td>
              <td style="font-weight : bold" class="school">
                <!-- <span>내 순위</span> -->
                {{ userSchool ? userSchool : '학교정보미입력' }}
              </td>
              <td style="font-weight : bold" class="nickname">{{ userName ? userName : '--' }}</td>
              <td
                class="score"
                style="font-weight : bold; padding-right: 15px;"
              >{{ numberWithCommas(myRanking.gamedata) }}</td>
              <!--   <td class="score" style="font-weight : bold; padding-right: 15px;">
                               {{ this.myRanking.rank === -1 ? '0' : numberWithCommas(this.myRanking.stage)}}
              </td>-->
            </tr>
            <tr v-else class="my-ranking">
              <td style="font-weight : bold" class="ranking">--</td>
              <td style="font-weight : bold" class="school">--</td>
              <td style="font-weight : bold" class="nickname">--</td>
              <!-- <td style="font-weight : bold" class="nickname">--</td> -->
              <td class="score" style="font-weight : bold; padding-right: 15px;">--</td>
            </tr>
            <tr v-for="(ranking, idx) in rankingList" :key="idx">
              ...
              >{{ numberWithCommas(ranking.gamedata) }}</td>
              <!-- <td class="score" style="padding-right: 15px;">{{ numberWithCommas(ranking.stage) ? numberWithCommas(ranking.stage) :'0'}}</td> -->
              <!-- <td class="typing-speed">{{ ranking.Speed }}</td> -->
            </tr>
          </thead>
        </table>
          ...
          <thead>
            ...
            <tr v-if="isLogin" class="my-ranking">
              ...
              <td
                style="font-weight : bold"
                class="school"
              >{{ userSchool ? userSchool : '학교정보미입력' }}</td>
              <td style="font-weight : bold" class="nickname">{{ userName ? userName : '--' }}</td>
              <td class="score" style="padding-right: 15px;">{{ numberWithCommas(myRanking.Score) }}</td>
              <!-- <td class="score" style="padding-right: 15px;">
                                {{ numberWithCommas(myRanking.Stage) ?  numberWithCommas(myRanking.Stage) : '0' }}
              </td>-->
            </tr>
            <tr v-else class="my-ranking">
              <td style="font-weight : bold" class="ranking">--</td>
            ...
            <tr v-for="(ranking, index) in rankingList[page - 1]" :key="index">
              ...
              <td class="nickname">{{ ranking.nickname ? ranking.nickname : '--' }}</td>
              <td class="score" style="padding-right: 15px;">{{ numberWithCommas(ranking.Score) }}</td>
              <!-- <td class="score" style="padding-right: 15px;">{{ numberWithCommas(ranking.Stage) ? numberWithCommas(ranking.Stage) : '0' }}</td> -->
              <!-- <td class="typing-speed">{{ ranking.Speed }}</td> -->
            </tr>
          </thead>
    ...
    <div v-if="$route.name === 'ranking-catching-moles-school'">
      <div class="ranking-content">
        <!-- <div class="ranking-week">
                    <div class="ranking-share">
          ...
          <thead>
            ...
            <tr class="my-ranking">
              <td
                ...
                style="font-weight : bold"
              >{{ myRanking.rank ? myRanking.rank : '--' }}</td>
              <td v-else class="ranking" style="font-weight : bold">--</td>
              <td v-if="isLogin" class="school" style="font-weight :bold">
                <!-- <span>내 순위</span> -->
                {{ userSchool ? userSchool : '학교정보미입력' }}
              </td>
              <td v-else class="school" style="font-weight :bold">
                <!-- <span>내 순위</span> -->
                --
              </td>
              <td
                ...
                style="padding-right: 15px;"
              >{{ numberWithCommas(myRanking.gamedata) }}</td>
              <td v-else class="score" style="padding-right: 15px;">--</td>
            </tr>
            <tr v-for="(ranking, idx) in rankingList" :key="idx">
              <td class="ranking">{{ ranking.rank ? ranking.rank : '--' }}</td>
              <td class="school">{{ ranking.schoolname ? ranking.schoolname : '학교정보미입력' }}</td>
        ...
        <table v-else class="ranker-table">
          <thead>
            ...
            <tr v-if="isLogin" class="my-ranking">
              <td class="ranking">{{ myRanking.Rank ? myRanking.Rank : '--' }}</td>
              <td class="school">{{ userSchool ? userSchool : '--' }}</td>
              <td class="score" style="padding-right: 15px;">{{ numberWithCommas(myRanking.Score) }}</td>
            </tr>
            <tr v-else class="my-ranking">
              <td style="font-weight:bold" class="ranking">
                <b>--</b>
              </td>
              <td style="font-weight:bold" class="school">
                <b>--</b>
...
export default {
  ...
  watch: {
    $route() {
      this.startRank;
      this.myRanking;
      if (this.$route.name === "ranking-catching-moles-personal") {
    ...
    shareKakao: function() {
      ...
      Kakao.Link.sendDefault({
        ...
        content: {
          ...
          link: {
            mobileWebUrl:
              "https://typing.malangmalang.com/rankings/catching-moles/personal",
            webUrl:
              "https://typing.malangmalang.com/rankings/catching-moles/personal"
          }
        },
...
#ranking .ranking-content .ranker-table td .score {
  text-align: right;
  padding-right: 30px;
}
.ranking-null {
