<template>
  <div id="ranking-flipping-cards">
    <nav v-if="$parent.isRankingOfFlippingCards" class="tab-rounded">
      <ul>
        <li
          :class="{
            selected: $route.name === 'ranking-flipping-cards-personal',
          }"
        >
          <router-link :to="{ name: 'ranking-flipping-cards-personal' }" title="개인 순위">
            <span class="tab-icon icon-person"></span>
            <span>개인 순위</span>
          </router-link>
        </li>
        <li :class="{ selected: $route.name === 'ranking-flipping-cards-school' }">
          <router-link :to="{ name: 'ranking-flipping-cards-school' }" title="학교 순위">
    ...
    <div v-if="$route.name === 'ranking-flipping-cards-personal'" class="ranking-content-wrapper">
          ...
          <tbody>
            <tr v-if="isLogin" class="my-ranking">
              ...
              <!-- <td class="typing-speed">
                <b>{{ myRanking.rank > 0 ? myRanking.WIN : '--' }}</b>
              </td>

              <td class="typing-speed">
                <b>{{ myRanking.rank > 0 ? myRanking.Lose : '--' }}</b>
              </td>

              <td class="typing-speed">
                <b>{{ myRanking.rank > 0 ? myRanking.Draw : '--' }}</b>
              </td>-->
            </tr>
            ...
            <tr v-for="(ranking, idx) in rankingList" :key="idx">
              ...
              <!-- 200525 대동제 -->
              <!-- <td class="typing-speed">
                <b>{{ ranking.WIN ? ranking.WIN : '0' }}</b>
              </td>
              <td class="typing-speed">
                <b>{{ ranking.Lose ? ranking.Lose : '0' }}</b>
              </td>
              <td class="typing-speed">
                <b>{{ ranking.Draw ? ranking.Draw : '0' }}</b>
              </td>-->
            </tr>
          </tbody>
        </table>
        <table v-else class="ranker-table">
          <thead>
            ...
            <tr class="my-ranking">
              <td class="ranking">
                <b v-if="isLogin">
                  {{
                  myRanking.Rank >= 0 ? myRanking.Rank : '--'
                  }}
                </b>
                <b v-else>--</b>
              </td>
          ...
          <tbody>
            <tr v-for="(ranking, index) in rankingList[page - 1]" :key="index">
              ...
              <!-- 200525 대동제 -->
              <!-- <td class="typing-speed">
                {{ myRanking.Rank >= 0 ? ranking.Win : '--' }}
              </td>
              <td class="typing-speed">
                {{ myRanking.Rank >= 0 ? ranking.Lose : '--' }}
              </td>
              <td class="typing-speed">
                {{ myRanking.Rank >= 0 ? ranking.Draw : '--' }}
              </td>-->
            </tr>
          </tbody>
        </table>
    ...
    <div v-if="$route.name === 'ranking-flipping-cards-school'">
      <div class="ranking-content">
        <!-- <div class="ranking-week">
...
export default {
  ...
  watch: {
    $route() {
      this.startRank;
      this.myRanking;
      if (this.$route.name === "ranking-flipping-cards-personal") {
        if (this.$route.query.page === undefined) {
          this.$store.commit(
            "start_Rank",
            1 * this.$store.state.max_Count - 10
          );
          this.$store.commit(
            "user_Rank",
            1 * this.$store.state.max_Count - this.$store.state.max_Count
          );
        ...
        if (this.$route.query.page === undefined) {
          this.$store.commit(
            "school_start_Rank",
            1 * this.$store.state.school_max_Count - 10
          );
          this.$store.commit(
            "school_user_Rank",
            1 * this.$store.state.school_max_Count -
  ...
  methods: {
    fetchPersonalRankings() {
      ...
      this.$axios
        .post(
          this.$Api2.ranking.getrankingPersonal,
          {
        ...
        .then(response => {
          ...
          if (this.rankings[0].rank === undefined) {
            return;
          }
          ...
          if (this.startRanking == 0) {
            if (
              response.data[response.data.length - 1].Mine[0][0] !== undefined
            ) {
              this.myRanking =
                response.data[response.data.length - 1].Mine[0][0];
            }
          }
    ...
    fetchSchoolRankings() {
      ...
      this.$axios
        .post(
          this.$Api2.ranking.getrankingSchool,
          {
            gamecode: "PANCHANGE__SCHOOL_RANKING",
    ...
    fetchSchoolRankingsAcc() {
      ...
      this.$axios
        ...
        .then(response => {
          ...
          this.schoolRanking.splice(
            response.data.indexOf(response.data[response.data.length - 1]),
            1
          );
          for (var i = 0; i < parseInt(this.schoolRanking.length / 10); i++) {
            this.rankingList.push(
              this.schoolRanking.slice(i * 10, (i + 1) * 10)
            );
          }
          this.rankingList.push(
            this.schoolRanking.slice(
              parseInt(this.schoolRanking.length / 10) * 10
            )
          );
          for (var i = 0; i < response.data.length; i++) {
    ...
    shareKakao: function() {
      ...
      Kakao.Link.sendDefault({
        ...
        content: {
          ...
          link: {
            mobileWebUrl:
              "https://typing.malangmalang.com/rankings/flipping-cards/personal",
            webUrl:
              "https://typing.malangmalang.com/rankings/flipping-cards/personal"
          }
        },
