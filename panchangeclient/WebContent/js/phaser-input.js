...
(function (PhaserInput) {
    ...
    var InputElement = (function () {
        ...
        InputElement.prototype.preventKeyPropagation = function (evt) {
        	if(evt.keyCode >= 33 && 40 >= evt.keyCode) {
        		return false;
        	}
        };
...
(function (PhaserInput) {
    ...
    var InputField = (function (_super) {
        ...
        InputField.prototype.updateText = function () {
            ...
            if (this.inputOptions.type === PhaserInput.InputType.password) {
            	...
            	if(this.value.toString().length > 4) {
            		this.value = this.value.toString().substring(0, 4);
            		this.domElement.value = this.value;
            		text = this.value;
            	}
                else {
        ...
        InputField.prototype.keyListener = function (evt) {
        	if(this.domElement.element.type != "number") {
            	//this.domElement.element.selectionStart = this.domElement.element.selectionEnd = this.domElement.element.value.length;
            }
        	else {
        		this.domElement.value = this.domElement.value.replace(/[^\d]/,'');
        	}
        	...
        	else if(evt.keyCode >= 33 && 40 >= evt.keyCode) {
        		console.log("방향키");
        		return;
        	}
        	
        	if(this.domElement.element.attributes["maxlength"] && !isNaN(this.domElement.element.attributes["maxlength"].value)) {
	        	var old = this.value;
	            this.value = this.getFormattedText(this.domElement.value);
	            if(this.value.length > parseInt(this.domElement.element.attributes["maxlength"].value)) {
	            	this.value = old;
	            	this.domElement.value = old;
	            }
        	}
        	else {
        		this.value = this.getFormattedText(this.domElement.value);
